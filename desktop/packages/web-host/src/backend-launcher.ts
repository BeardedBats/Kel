...
export class BackendLifecycleManager {
  ...
  private async attemptStart(
    ...
    console.log('[KEL-BOOT] backend-launcher: spawning aioncore');
    ...
    try {
      ...
      console.log('[KEL-BOOT] backend-launcher: startup directory prep FAILED');
      this._status = 'error';
    ...
    try {
      ...
      console.log('[KEL-BOOT] backend-launcher: spawn() threw');
      this._status = 'error';
      throw makeStartupError('spawn', 'aioncore process spawn threw before startup', error);
    }
    console.log(`[KEL-BOOT] backend-launcher: aioncore spawned pid=${this.childProcess.pid}`);
    this.childProcess.on('error', (error) =>
      console.log(`[KEL-BOOT] backend-launcher: child error ${String(error)}`)
    );
    this.childProcess.on('exit', (code, signal) =>
      console.log(`[KEL-BOOT] backend-launcher: child exit code=${code} signal=${signal}`)
    );
