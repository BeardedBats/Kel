 ...
 * ```
 */
const SettingsModal: React.FC<SettingsModalProps> = ({ visible, onCancel, defaultTab = 'system' }) => {
  ...
  const menuItems = useMemo((): Array<{ key: SettingTab; label: string; icon: React.ReactNode }> => {
    ...
    // Kel V1.2: the donor's model/tools tabs configure AionUI's multi-assistant
    // stack, which Kel does not use — hidden so Settings only shows what applies.
