...
function buildBuiltinBrowserServer(): McpImportServer {
  ...
  return {
    ...
    description:
      "Control Kel's built-in browser (the side preview panel): open pages, click, type and read content. " +
...
async function syncBuiltinMcpConfig(configFile: ConfigFile): Promise<void> {
  ...
  console.info(
    '[Kel] Synced builtin MCP config to backend settings (%d builtin servers)',
...
export async function runBackendMigrations(configFile: ConfigFile): Promise<void> {
  await CLEANUP_STEPS.reduce<Promise<void>>(async (previous, step) => {
    ...
    try {
      await step.run();
      console.info(`[Kel] Backend migration step completed: ${step.name} (${Date.now() - start}ms)`);
    } catch (error) {
      console.error(`[Kel] Backend migration step failed: ${step.name} (${Date.now() - start}ms)`, error);
    }
  }, Promise.resolve());
  ...
  await MIGRATION_STEPS.reduce<Promise<void>>(async (previous, step) => {
    ...
    try {
      ...
      if (!completed) {
        console.warn(`[Kel] Backend migration step incomplete: ${step.name} (${elapsed}ms)`);
        return;
      }
      console.info(`[Kel] Backend migration step completed: ${step.name} (${elapsed}ms)`);
    } catch (error) {
      const elapsed = Date.now() - start;
      console.error(`[Kel] Backend migration step failed: ${step.name} (${elapsed}ms)`, error);
    }
  }, Promise.resolve());
  ...
  try {
    ...
    console.info(`[Kel] Backend migration step completed: syncBuiltinMcpConfig (${Date.now() - syncStart}ms)`);
  } catch (error) {
    console.error(`[Kel] Backend migration step failed: syncBuiltinMcpConfig (${Date.now() - syncStart}ms)`, error);
  }
}
