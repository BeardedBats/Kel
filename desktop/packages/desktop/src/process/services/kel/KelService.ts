...
function spawnEngine(root: string): void {
...
async function waitForEngineReady(descriptorPath: string): Promise<boolean> {
...
async function performEngineRestart(root: string, descriptorPath: string): Promise<void> {
  ...
  try {
    ...
    spawnEngine(root);
    const ok = await waitForEngineReady(descriptorPath);
...
export async function initializeKel(port: number): Promise<void> {
  ...
  if (!connected) {
    ...
    if (!(await waitForEngineReady(descriptorPath))) throw new Error('Kel engine did not start. See desktop.log.');
  }
