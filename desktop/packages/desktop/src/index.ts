// Modified for Kel: bootstrap its sole ACP assistant.
/**
 * @license
...
// ============ [KEL-BOOT] temporary startup diagnostics ============
const kelBootStart = Date.now();
let kelBootSeq = 0;
function kelBoot(msg: string): void {
  console.log(`[KEL-BOOT] ${String(++kelBootSeq).padStart(3, '0')} +${Date.now() - kelBootStart}ms ${msg}`);
}
process.on('exit', (code) => kelBoot(`process.exit code=${code}`));
process.on('uncaughtException', (error) => kelBoot(`uncaughtException: ${String(error)}`));
process.on('unhandledRejection', (reason) => kelBoot(`unhandledRejection: ${String(reason)}`));
app.on('before-quit', () => kelBoot('app event: before-quit'));
app.on('will-quit', () => kelBoot('app event: will-quit'));
...
kelBoot(`gotTheLock=${gotTheLock} skipSingleInstanceLock=${skipSingleInstanceLock}`);
if (!gotTheLock) {
  kelBoot('lock lost -> app.quit()');
  console.warn('[Kel] Another instance is already running; current process will exit.');
  app.quit();
} else {
  app.on('second-instance', (_event, argv, _workingDirectory, additionalData) => {
    ...
    if (app.isReady()) {
      console.log('[Kel] second-instance received; focusing existing main window');
      showOrCreateMainWindow({
        mainWindow,
        createWindow: () => {
          console.log('[Kel] second-instance received with no active main window, recreating main window');
          createWindow();
        },
      });
    }
...
if (electronSquirrelStartup) {
  kelBoot('electronSquirrelStartup=true -> app.quit()');
  app.quit();
}
kelBoot('squirrel check passed; registering error handlers');
...
function logUncaught(diagnostics: UncaughtErrorDiagnostics): void {
  try {
    console.error(`[Kel] ${diagnostics.origin}:`, diagnostics);
  } catch {
...
function registerCronResumeBridge(backendPort: number): void {
  ...
  const onResume = () => {
    void fetch(`http://127.0.0.1:${backendPort}/api/cron/internal/system-resume`, {
      ...
      console.error('[Kel] Failed to notify backend about system resume:', error);
    });
  };
...
const scheduleBackendMigrations = (): void => {
  ...
  void (async () => {
    try {
      ...
      console.info('[Kel] runBackendMigrations completed');
    } catch (error) {
      console.error('[Kel] Backend migration hook threw:', error);
    }
  })();
};
...
function resolveDebugBackendStartupFailure(): BackendStartupFailureInfo | null {
  ...
  if ((app.isPackaged && !isE2ETestMode) || isWebUIMode || isResetPasswordMode) {
    console.warn('[Kel] Ignoring AIONUI_DEBUG_BACKEND_STARTUP_FAILURE outside desktop dev/e2e mode.');
    return null;
  }
  ...
  console.warn(`[Kel] Ignoring unknown AIONUI_DEBUG_BACKEND_STARTUP_FAILURE value: ${reason}`);
  return null;
}
...
const createWindow = ({ showOnReady = true }: { showOnReady?: boolean } = {}): void => {
  ...
  if (!disableAutoUpdater) {
    ...
    console.log('[Kel] Auto-updater disabled via env/CI guard');
  }
  ...
  if (!app.isPackaged && rendererUrl) {
    console.log(`[Kel] Loading renderer URL: ${rendererUrl}`);
    mainWindow.loadURL(rendererUrl).catch((error) => {
      console.error('[Kel] loadURL failed, falling back to file:', error.message || error);
      mainWindow.loadFile(fallbackFile).catch((e2) => {
        console.error('[Kel] loadFile fallback also failed:', e2.message || e2);
      });
    });
  } else {
    console.log(`[Kel] Loading renderer file: ${fallbackFile}`);
    mainWindow.loadFile(fallbackFile).catch((error) => {
      console.error('[Kel] loadFile failed:', error.message || error);
    });
  }

  mainWindow.webContents.on('did-fail-load', (_event, errorCode, errorDescription, validatedURL, isMainFrame) => {
    console.error('[Kel] did-fail-load:', { errorCode, errorDescription, validatedURL, isMainFrame });
  });
  ...
  mainWindow.webContents.on('render-process-gone', (_event, details) => {
    console.error('[Kel] render-process-gone:', details);
    if (mainWindow.isDestroyed()) return;

    const action = rendererRecovery.onCrash(details.reason);

    if (action.kind === 'relaunch') {
      console.warn(`[Kel] renderer cannot be recovered in-place (reason=${details.reason}); relaunching app`);
      app.relaunch();
      app.exit(0);
      return;
    }
    ...
    if (action.kind === 'give-up') {
      console.error(`[Kel] renderer recovery exhausted (reason=${details.reason}); not retrying`);
      return;
    }
    ...
    const reload = () => {
      if (mainWindow.isDestroyed()) return;
      console.log('[Kel] Attempting to recover from renderer crash by reloading...');

      if (!app.isPackaged && rendererUrl) {
        mainWindow.loadURL(rendererUrl).catch((error) => {
          console.error('[Kel] Recovery loadURL failed:', error.message || error);
        });
      } else {
        mainWindow.loadFile(fallbackFile).catch((error) => {
          console.error('[Kel] Recovery loadFile failed:', error.message || error);
        });
      }
    };
  ...
  mainWindow.webContents.on('unresponsive', () => {
    console.warn('[Kel] Renderer became unresponsive');
  });

  mainWindow.on('closed', () => {
    console.log('[Kel] Main window closed');
  });
...
const handleAppReady = async (): Promise<void> => {
  kelBoot('handleAppReady entry');
  const t0 = performance.now();
  const mark = (label: string) => console.log(`[Kel:ready] ${label} +${Math.round(performance.now() - t0)}ms`);
  mark('start');
  ...
  if (debugBackendStartupFailure) {
    ...
    kelBoot('before startBackendOrExit');
    ...
    kelBoot(`backendStartup ok=${backendStartup.ok}${backendStartup.ok ? ' port=' + backendStartup.port : ''}`);
    if (!backendStartup.ok) {
      if (isWebUIMode || isResetPasswordMode) {
        kelBoot('backendStartup failed + webui/resetpass mode -> return');
        return;
      }
    }
    ...
    if (backendStartedOk && bootBackendPort) {
      ...
      kelBoot(`skipping Kel init: backendStartedOk=${backendStartedOk} bootBackendPort=${bootBackendPort}`);
    }
  }
  ...
  try {
    ...
    console.error('[Kel] Failed to restore window bounds:', error);
  ...
  if (isResetPasswordMode) {
    ...
    kelBoot('before createWindow');
    createWindow({ showOnReady: showMainWindowOnReady });
    kelBoot('after createWindow');
    appReadyDone = true;
    mark('createWindow');
...
if (shouldRegisterBackendStartup(gotTheLock)) {
  kelBoot('registering whenReady -> handleAppReady');
  void app
    .whenReady()
    .then(() => {
      kelBoot('whenReady fired');
      return handleAppReady();
    })
    .catch((error) => {
      kelBoot(`handleAppReady rejected: ${String(error)}`);
      // App initialization failed
      console.error('[Kel] App initialization failed:', error);
      app.quit();
    });
} else {
  kelBoot('backend startup NOT registered (no lock)');
}
...
app.on('will-quit', () => {
  console.log('[Kel] will-quit — all cleanup should be complete');
});

app.on('quit', (_event, exitCode) => {
  console.log(`[Kel] quit (exitCode=${exitCode})`);
});
