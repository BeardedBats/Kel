...
Suites: `runtime/tests/test_transcription.py` (26 tests, all against the deterministic fixture
...
| export audio (and plain error when absent) | `test_export_text_…` / `export_audio` in service tests | PASS |
...
| **architectural contract**: review route == direct ingestion | `test_transcript_route_matches_direct_ingestion` | PASS |
