...
| `fde5bbb` | `eea6503` | 2026-09-18 | R2 / EVENT-IDEMPOTENCY + EFFECT-REPLAY | observed external effect keeps its receipt; the 17-family idempotency matrix recorded (16 safe, 1 latent gap repaired) | `kel/core.py` (`observe_effect`); `tests/test_v16_r2_idempotency.py`; `docs/v1.6/pre-audit/increments/R2-IDEMPOTENCY-MATRIX.md` | +10 engine tests; full suite A-22 | none | no | n/a | no | yes (effect receipt rule) | MEDIUM | `increments/R2-IDEMPOTENCY-MATRIX.md`; INV-EVENT-IDEMPOTENCY; INV-EFFECT-REPLAY; REQ-R25-R2 | `observe_effect` had no production caller (latent); reconciliation entry point for a legitimately revised observation is a recorded audit question |
