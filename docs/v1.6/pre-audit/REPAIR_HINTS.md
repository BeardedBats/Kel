...
| APR-03 vetting→memory queue | `runtime/kel/vetting_session.py` (~line 95), `runtime/kel/service.py` (~682-688) | `_memory_check_queue`, `flush_memory_checks` | `test_v16_proposals.py` (vetting section); add crash-window test | Direction: record a durable `memory_check_pending` marker (existing `vetting_events` table) inside `_decision_for`'s transaction; make `flush_memory_checks` drain pending markers lacking a processed marker (proposal dedupe makes re-processing safe); verify a decision confirmed before a simulated restart still queues its proposal | INV-MEM-001 |
