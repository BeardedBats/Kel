...
| 5b | **Allow once** — one action, and the grant reaches the policy | the control's Allow once makes chat A report `usable: true` while disabled; the single-use spend is proven at the boundary | `chatAAllowOnceUsable: true`; `test_allow_once_is_spent_by_the_authorization_path` (pre-flight sees it, the effect spends it, the next request is refused) |
