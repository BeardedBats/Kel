...
**JR-55 - Approvals answer in the conversation, on one durable record.** Anything Kel is waiting on that the user can decide right now (access to a folder outside the repository it was trusted with, or a command it wants to run) appears as a card in the conversation itself: what Kel wants, why, and two or three plain choices ("Allow once", "Allow for this project", "Deny", "Approve", "Always allow", "Details"). The card resolves the same durable record the Work surfaces show - approving in chat is visible in Work and vice versa - settles in place once decided (a sentence like "Allowed once - Kel is continuing.", never dead buttons), refuses double-submits and stale asks honestly ("Expired"), and no approval ids, leases or scopes appear outside Details. Denying changes nothing else, is explained in one plain sentence, and the identical request is never asked again. Verify: in the packaged app trigger a real approval, resolve it from the chat card (the durable record flips once, the paused job continues, the card settles), resolve a second one from the Work drawer (the chat card follows), deny one and restart with one still open; the engine suite pins resume/deny/expiry/double-resolution behavior.
