...
| G6 — providers / credentials / autonomy | **IN PROGRESS** | provider registry + capability matrix + full state model (incl. quota-not-reported) + readiness preflight with recorded fallback reasons + credential **metadata** (migration 007) + `/api/providers` landed with 20 PROV-* tests (suite 316 + 10). Remaining: OS-backed credential custody via the shell, autonomy engine (leases/guardrails) + AUTO-* tests, Providers/Autonomy surfaces |
