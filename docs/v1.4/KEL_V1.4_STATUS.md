...
| G7 — desktop productization | **IN PROGRESS** | first increment: the dead settings routes (`model`/`tools`/`skills`/`agent`/`capabilities`/`skills-hub`) now land on real Kel surfaces (capture hashes `#/providers`, `#/autonomy`, `#/team/roster`; byte-identical renders) and the muted-text contrast was repaired at the source (`--bg-6`, `.arco-btn-outline`, `.assistantPromptHint`, `.workspaceEmptyBtn`) — audit contrast failures **6 → 0** with smallest text 12px and 0 emoji; probe now records the failing element. Remaining: onboarding, search + command palette, tray/notification/pet tokenization, shell keyboard pass |
