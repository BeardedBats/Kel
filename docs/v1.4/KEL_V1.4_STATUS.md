...
| G7 — desktop productization | **IN PROGRESS** | fixed: dead settings routes → real Kel surfaces (`/providers`, `/autonomy`, `/team/roster`); **focus rings 0/30 → 30/30**; **contrast 6 → 0**; skip link proven as the first tab stop on a fresh load (visible + ringed); **command palette** (`Ctrl+K`, `/`) with 24 engine-derived results and keyboard-only navigation verified (Enter → `#/providers`); Kel tokens load globally. Open: donor drawer tabs are `DIV`s; migrated profiles see the flow once (flag-only rule, recorded deviation). Tray/notification restraint verified in code; Sider duplicate label fixed. **Gate 7 complete pending its reviewer relay** |
...
1. Start **Gate 6**: providers / credentials / autonomy — provider registry + capability matrix +
   readiness preflight (installed · not authenticated · quota · not-reported · degraded states),
   OS-backed credential custody via the shell (engine keeps metadata only), DeepSeek first-class card,
   capability-lease + boundary-expansion UI, and AUTO-* enforcement tests for the locked red lines.
