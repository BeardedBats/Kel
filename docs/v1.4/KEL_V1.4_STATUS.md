...
| G7 — desktop productization | **IN PROGRESS** | fixed: dead settings routes now land on real Kel surfaces (capture hashes `#/providers`, `#/autonomy`, `#/team/roster`) and **focus rings 0/30 → 30/30** tab stops (`solid 2px rgb(14,124,90)`), contrast failures **6 → 0** (smallest text 12px, 0 emoji), Kel tokens now load globally (single token source), probe records the failing element and resets focus before the tab pass. Open: skip link not yet measurable in the tab order, donor drawer tabs are DIVs. Remaining: onboarding, search + command palette, tray/notification/pet tokenization |
