...
| G6 — providers / credentials / autonomy | **IN PROGRESS** | engine complete: provider registry + capability matrix + full state model (incl. quota-not-reported) + readiness preflight + credential **metadata** (migration 007) + `/api/providers` (20 PROV-*) · autonomy engine: capability leases with scope enforcement, boundary expansion (ask once / project / deny), locked-guardrail tamper detection, emergency stop, `/api/autonomy` (30 AUTO-*) — suite **346 + 10**. UI landed: `/providers` + `/autonomy` rendered from the packaged candidate (tags `g6`/`g6b`, 0 errors, 0 blank, route contrast 0/0), engine rebuilt + PYZ-verified. Remaining: OS-backed credential custody via the shell |
