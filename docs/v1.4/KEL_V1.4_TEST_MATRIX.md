...
| G7-SEMANTICS | donor drawer controls use semantic elements | I | **OPEN (evidence disagrees)** — the audit flags “DIVs with `tabindex`”. Two measurements conflict: Arco's library source assigns WAI-ARIA to its tab titles (`…/Tabs/tab-header/tab-title.js`: `role: "tab"`, `aria-selected`, `aria-controls`, `tabIndex`, `onKeyDown`), but the V1.4 capture's focused elements report **`role=None`** on all seven stops (`Work`, `Saved context`, `Saved history`, `Continue work`, `Project knowledge`, `Project map`, `Recipes`). Either the sampled nodes are inner wrappers or the app renders a custom tab strip. **Next step (bounded)**: inspect those nodes' outer HTML in a devtools-level probe (or check whether the drawer overrides Arco's Tabs header) and then either add the missing roles or close it as a sampling artefact. No refactor before that; they are reachable, ringed, and keyboard-operable today |
