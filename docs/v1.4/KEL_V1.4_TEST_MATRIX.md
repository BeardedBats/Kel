...
## 18. Gate 10 additions

| Id | Check | Level | Status |
|---|---|---|---|
| G10-UPGRADE | a real V1.3 store (schema_migrations 1–4, none of the V1.4 tables) upgrades additively | U | **PASS** — `tests/test_v14_upgrade.py` (5 UPG-* tests): all **22** V1.4 tables appear, migrations **5–9** are recorded with their names, every pre-existing job and memory row is unchanged, a pre-migration backup exists, and reopening is idempotent |
| G10-UPGRADE-FEATURES | V1.4 features run on upgraded data | U | **PASS** — on the upgraded store: roles seed, a solution brief opens, a capability lease issues against the pre-existing job, and the diagnostics snapshot reports integrity `ok` with the upgraded job counted |
| G10-SUITE | full engine suite | U | **PASS** — **363 passed + 10 subtests** (358 + 5 UPG-*) |
