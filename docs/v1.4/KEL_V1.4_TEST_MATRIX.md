...
| G9-DARK-REMAINDER | remaining dark failures, precisely attributed | V | **OPEN (3 labels)** — `sider-section-label` (“Project conversations”, 2.53:1), the Arco text button “Work & context” (2.13:1) and the workspace footnote (“Work in a project”, 2.46:1) keep **hardcoded donor colours** that no token reaches (`rgb(92,100,112)` / `rgb(78,89,105)` measured identically in both themes). Fix direction: correct those three component rules directly; note the light theme is unaffected (0 failures) |
