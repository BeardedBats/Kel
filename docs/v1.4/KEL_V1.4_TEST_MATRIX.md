...
| G7-SIDER | navigation matches the V1.4 information architecture | I | **PASS** — the Sider renders exactly the Kel entries in order (`Work · Team · Projects · Providers · Autonomy`) ahead of the donor conversation list and Settings; after fixing the donor group label (`conversation.projectsSection`: `Projects` → `Project conversations`) the rendered text contains **exactly one** “Projects” token (verified by count in `g7/g7sider-texts.jsonl`) |
