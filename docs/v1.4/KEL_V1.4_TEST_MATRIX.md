...
| G7-ONBOARDING-GATE | the automatic first-run trigger fires on a genuinely fresh install | I/V | **OPEN (not passed)** — with a brand-new data root the app stays on `#/guid` for the whole 20 s polled timeline (recorded in `screenshots/audit/v14/onboarding/onboarding-evidence.json`); a migrated root correctly stays on `#/guid`. Engine reads are NOT the cause: on the same fresh root the palette resolves 14 navigation/role entries with 0 errors (`api/state`, `/api/work`, `/api/team` all answer). Next step: log the gate's two decision inputs (flag value + conversation count) in a probe build, or read the donor's own conversation list instead of `/api/state` |
