...
| G9-DARK-REMAINDER | remaining dark failures, precisely attributed | V | **PARTIAL (3 labels + Arco components)** — route failures improved (work 0, providers 4, autonomy 2, diagnostics 2) and light stays 0/0/0/0, but three labels remain because **Arco resolves its text colours from React-injected variables, not from the attributes the harness sets**: the token probe reads `--color-text-2`/`--color-text-3` from the root as **empty strings**, so Arco's compiled fallbacks (measured `rgb(90,98,112)` = `#5a6270`, and `rgb(92,100,112)` = the light muted value) win inside components. The same-precedence fixes for `.sider-section-label`, `.workspaceFootnote` and the chat trigger are in place and correct for the CSS layer. **Next step (bounded)**: drive the app's own Appearance setting (or set Arco's variables on `body`) before capturing, instead of switching attributes — that is the blocker for a fully dark-component capture, and it is a harness/app-setting task rather than a token task |
