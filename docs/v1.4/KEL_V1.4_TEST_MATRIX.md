...
| G7-SEMANTICS | donor drawer controls use semantic elements | I | **PASS (false positive, closed)** — the audit flagged “DIVs with `tabindex`” in the work drawer. Measured against the library source, Arco's tab titles are correct WAI-ARIA tabs (`node_modules/@arco-design/web-react/es/Tabs/tab-header/tab-title.js`: `role: "tab"`, `aria-selected`, `tabIndex: disabled ? -1 : 0`, `id`/`aria-controls`, `onClick`, and `onKeyDown` with key handling), and the shell focus rule now rings them. No refactor was warranted; the panel's own controls are Arco `Button`s |
