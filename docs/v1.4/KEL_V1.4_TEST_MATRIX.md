...
| G7-SIDER | navigation matches the V1.4 information architecture | I | **PASS with one note** — the Sider renders exactly the Kel entries in order (`Work · Team · Projects · Providers · Autonomy`) ahead of the donor conversation list and Settings, captured in `g7/g7onb-texts.jsonl` (`01-boot-chat`). Note (open): the token “Projects” appears twice in the rendered text — once for the Kel nav entry, once for a donor conversation-group heading rendered by a child component outside `components/layout/Sider` — so two different controls share a name; fix direction is to rename the donor heading once its component is located (no blind patch made) |
