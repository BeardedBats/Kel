...
| 6 | Read-only native agent launch (chat/review/text) | `native.NativeAdapter.execute` ← chat path, Commander review, durable text jobs | `user`/`kel`/`worker` | none — **explicitly lease-free**: no repository/filesystem effects; tools disabled by argv; `child_env` strips the other provider's key and `CLAUDECODE` | `Popen(claude/codex)` | **TRUSTED-RUNTIME** (read-only) | `NativeTrustBoundaryTests` + `ChildEnvironmentTests` (`test_v15_credentials.py`) |
...
| 26 | ACP presentation adapter | `acp_host.py` stdio (launched by the desktop shell) | proxies to the authenticated HTTP service | owns no engine; every request re-enters the service boundary; donor-agent `session/request_permission` requests are refused by policy (V1.5 G12) and no tool execution is implemented on this surface | none (HTTP client) | presentation only | source; used via `--acp` |
...
| 28 | WSL distro/setup utility (`runtime_setup.py`) | explicit setup utility | `user` (out of band) | no callers found in the running product's source; flagged for the G8 packaging/donor audit | downloads/imports distro; writes provider auth into the isolated distro root-only | **TRUSTED-RUNTIME (setup)** — credential aspect reviewed at G4 (`04_CREDENTIAL_RUNTIME.md`): out of the host env path; re-checked at G8/G9 | source; `runtime_setup.py` |
| 29 | Provider credential injection at engine spawn | `KelService.ts` spawn ← desktop main process decrypts from OS secure storage | `user` (stored credential; decrypted only in the main process) | custody + injection path (`04_CREDENTIAL_RUNTIME.md`); no IPC returns values | engine child env only; durable text redacted | **CONTROLLED (G4)** | `test_v15_credentials.py` (6 tests: `RedactionTests`, `RequestScopeTests`, `ChildEnvironmentTests`, `DurableLeakTests`); packaged check at G11 |
