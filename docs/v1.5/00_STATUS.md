...
| G11 migration / clean clone / packaged | **closed** — migration matrix measured (`13`): fresh ✓, **real V1.3-origin root upgraded through V1.4 → V1.5 `RESULT: OK`**, synthetic pre-010 `test_v15_upgrade.py` 1/1, retained upgrade suites 7/7; engine PYZ `RESULT: OK` (34 modules); **clean clone verified end-to-end**: `git clone --no-hardlinks` → `bun install --frozen-lockfile` (1591 packages) → renderer build / tsc / vitest / `electron-builder` (pinned-ABI path) / engine rebuild / PYZ / smoke (`engineVersion: 1.5.0`, engineStopped, appExited) / packaged-UI (all checks, `consoleErrors: []`) all green (packaging order fixed: engine build precedes packaging). Candidate-vs-assembled comparison performed at G13 on the frozen copy | `13_MIGRATIONS.md`, `packaging/verify-live-upgrade.py` |
| G12 independent architecture audit | **closed** — independent read-only audit returned **REVISE** (F1 ACP permission surface, F2 Autonomy copy scope) → remediation `ed0b35e` (explicit ACP permission denial; copy names the live effect points; doc corrections F3/F4/F6; version identity F7; F5 rebutted with `coding.py:161-170` tool= intents + `test_a_strict_snapshot_blocks_the_coding_dispatcher`) → tight re-audit: all seven items CONFIRMED, **VERDICT: CONTINUE** | audit transcripts; `ed0b35e` |
| G13 freeze / tag / release | **closed — RELEASED** — final candidate rebuilt in order (engine → renderer → package), probes green (smoke / packaged-UI / a11y `contrastFailureCount: 0`), **clean clone at the release commit green across all nine stages**, frozen at `Kel Releases\Kel-V1.5-Frozen` with `verify-release.ps1` **3/3** at the frozen location, candidate↔frozen `Kel.exe` **byte-identical**, prior frozen releases re-verified 3/3 and byte-untouched; **tag `v1.5.0`**; manifest `15`, evidence `evidence/g13/`; ledger reconciliation explicit (`08`) incl. "Surface rows deferred out of V1.5" (`16`) | `15_RELEASE_MANIFEST.md`, `evidence/g13/` |
...
`15_RELEASE_MANIFEST` ✅ released · `16_KNOWN_LIMITATIONS` ✅ working · `17_V2_PLUS_DEFERRED` ✅ ·
`AUTO_RESUME` ✅
