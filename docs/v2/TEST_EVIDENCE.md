...
[Model Add model `311:2730` and populated custom rows](evidence/figma-full-audit/DESKTOP_MODEL_CURRENT_REVISION.md) were implemented as one desktop batch. A disposable package placed the 560×448 dialog at x440/y130 at 1440px; it fit 800px. A fake key and loopback endpoint saved two models. The populated card measured 670×209 at 1440px and 388×209 at 800px. Provider and model switches toggled off and on; the options menu opened. Check attempted only loopback and recorded a failed result. No overflow or renderer error appeared. TypeScript, **54 files / 401 tests**, Electron Vite, and Windows packaging passed. Canonical App and Data were untouched.
