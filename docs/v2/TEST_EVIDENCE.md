...
The canonical installed App ran with disposable data for desktop Dark Tools at 1440 and 800px. Fresh disconnected MCP rows, an active retest, and a 503 failure/help detail rendered without page errors or document overflow. The stand-in server was bound to loopback; it was stopped with the app, and the temporary data root was removed. [Captures and limits](evidence/figma-full-audit/DESKTOP_TOOLS_STATUS_CANONICAL.md) separate this packaged behavior from the subsequent source styling repair. After that repair, desktop typecheck and unit tests passed again: **45 files / 298 tests**. The repair has not yet received packaged visual proof.
