...
Desktop Workspace and File preview now use current Figma surfaces, local assets, file tabs, toolbar, real file actions, and truthful details. Source-render checks pass at 1440/800px, including narrow composer and pane clipping repairs. Final isolated package verification is pending. See [Workspace and File preview](evidence/figma-full-audit/DESKTOP_WORKSPACE_CURRENT_REVISION.md).

Desktop Model and Project pickers now use current Figma glass menus, local icons, scope tabs, model actions, and project search. The model control sits inside the composer's right side. Real isolated preference writes and restoration passed; project browse used an injected native dialog result. All 408 desktop tests and 1440/800px package checks passed. See [Desktop pickers](evidence/figma-full-audit/DESKTOP_PICKERS_CURRENT_REVISION.md). Workspace and File preview are now implemented; final package checks are next. Mobile remains paused.

Desktop Fix Capture now follows current Select, Recording, and Review frames. Its amber target persists; the glass panels, review field, dot asset, timer, and action order match their scoped Figma components. Tooltips stay hidden during capture. The isolated package checked both widths and the synthetic recording/retry/cancellation lifecycle. The palette focus-ring and delayed-search repairs have package proof. See [Desktop Fix Capture](evidence/figma-full-audit/DESKTOP_FIX_CAPTURE_CURRENT_REVISION.md).

Desktop Command palette, Chat row menu, and Rename chat now use the current Figma structure and local icon assets. Palette groups contain actual navigation, actions, recipes, and chats. Chat Export now loads full history and creates Markdown. The isolated package checked both desktop widths, Escape, and Capture a fix cancellation. A download probe checked synthetic export content. The final palette focus-ring CSS repair has source-render proof; the next batch will confirm it in a package. See [scoped evidence](evidence/figma-full-audit/DESKTOP_CHAT_OVERLAYS_CURRENT_REVISION.md).

Desktop New scheduled task and Approval details now follow current Figma's overlay layouts. The new-task form groups Name and Assistant, shows desktop frequency tabs and the skip switch, and keeps the existing phone controls. Approval details show the five Figma sections with truthful fallback text when the engine omits optional fields. A disposable package measured both at 1440/800px; no task or approval was submitted. See [scoped evidence](evidence/figma-full-audit/DESKTOP_OVERLAYS_CURRENT_REVISION.md). The canonical App remains on source `8c67121` until the desktop completion install.

Desktop Archived now follows current Figma grouped and selected cards. A disposable package checked 1440/800px with synthetic archive rows and canceled the delete confirmation. Enabled WebUI now uses Figma's service, sign-in, phone-code, and password-mismatch layouts. An isolated package started and stopped the server; no real credential changed. Desktop System, Restore, and About follow current Figma cards and dialog geometry. A disposable package inspected a real isolated backup, canceled the confirmation, and opened Third-party notices. Desktop Appearance reserves a fourth theme slot for a real saved theme, shows six primary colors, and groups Chat text, Code, and Zoom. Add theme uses the compact current Figma field order. A separate isolated custom theme survived selection and reload. Advanced colors remain available. Other desktop frames remain open. The canonical App remains on source `8c67121` until the desktop completion install.
