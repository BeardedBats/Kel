...
The Projects empty map card now matches Figma's 85px height while retaining Refresh map on hover or keyboard focus. Providers' readiness card now shows three truthful rows, including an explicitly untested model reply. Model has Figma's default-model row structure and real Use, Automatic, and Set up actions. Isolated package checks covered both at 1440 and 800px. Populated custom models and Add model overlay parity remain open. Diagnostics' cache/restart controls remain disabled because the runtime does not support their stated behavior safely.
