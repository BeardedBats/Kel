...
**Next item:** Continue desktop first: saved Knowledge/project recipes, then remaining startup and shared-dialog frames. Active/waiting Work and populated Permissions pass isolated engine source checks at 1440/800px. Auto Edit and Add a file labels match. Package at a larger desktop milestone; canonical App remains 8c67121. Keep mobile paused. Pet enable requires Nick's policy decision. Diagnostics maintenance needs matching runtime operations. See [evidence](evidence/figma-full-audit/DESKTOP_WORK_PERMISSIONS_STATES.md). The bounded renderer stays open during continuation; all new jobs were canceled/acknowledged and leases revoked.
