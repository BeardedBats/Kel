...
**Next item:** Desktop first: complete the combined menus/Work/Permissions/Knowledge milestone package check, then startup and shared-dialog frames. Knowledge and project recipes pass isolated engine source checks; 427 desktop tests pass. Canonical App stays 8c67121 until broader desktop completion. Keep mobile paused. Pet enable requires Nick's policy decision; diagnostics maintenance requires matching engine operations. See [Knowledge evidence](evidence/figma-full-audit/DESKTOP_KNOWLEDGE_CURRENT_REVISION.md).
