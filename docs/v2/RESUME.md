...
**Next item:** desktop first. Workspace and File preview have final package checks. Activity loading `273:12291` and Providers error `273:12486` now have [package evidence](evidence/figma-full-audit/DESKTOP_PENDING_STATES_CURRENT_REVISION.md). Memory review, Permission, Slash, and Attach styling now has [source-render evidence](evidence/figma-full-audit/DESKTOP_CHAT_MENUS_CURRENT_REVISION.md). Resolve its two label differences within the next desktop group. Then verify populated desktop Work, Permissions, Projects, and Scheduled tasks. Nick chose source/render checks between larger desktop packaging milestones on 2026-09-26. Commit and push verified increments. Keep mobile paused until desktop implementation is materially complete. Canonical App promotion follows desktop completion.
