...
| Populated mobile Knowledge | Mobile Knowledge `299:14371` shows two suggestions and three map rows | The isolated package has no proposals or map. Create bounded engine fixtures, then compare real review actions and populated cards. Do not ship Figma's sample choices as user data. | `projects/index.tsx`, engine memory and project map |
