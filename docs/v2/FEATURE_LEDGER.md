...
| V2-04 | Connection Framework + templates | **PARTIAL** — templates, request policy/retries and actions (rows + run + confirmation gate + access history) are built; a chat tool for actions and the OAuth sign-in step are not | `runtime/kel/connection_framework.py`, `connection_actions.py`, `connections.run()`/`events()` (migration 26) + 16 engine tests; "What Kel can do" on the page + 5 desktop tests |
