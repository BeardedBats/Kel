...
| V2-04 | Connection Framework + templates | **PARTIAL** — templates + request policy/retries built; actions/tools and the OAuth sign-in step are not | `runtime/kel/connection_framework.py` (three templates, request policy) + `perform_request` retries (7 engine tests); the renderer's kind vocabulary deleted in favour of the framework's + 3 desktop tests |
