...
next_item: Complete desktop implementation first. Implement Fix Capture Select/Recording/Review (273:906, 273:1091, 273:1338) as one desktop batch. Include packaged confirmation of the command-palette focus-ring CSS repair. Then close remaining chat pickers and populated Work, Permissions, Projects, and Scheduled tasks with bounded engine states. Desktop Pet enable remains locked pending Nick's V1.6 policy decision; verify its truthful off state separately. Diagnostics maintenance remains open until matching runtime operations exist. Keep mobile paused. Update canonical App for Nick to test after desktop completion, then resume mobile UI/UX.
