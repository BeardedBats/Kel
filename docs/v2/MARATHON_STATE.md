...
next_item: acceptance infrastructure (V2-18) # V2-17 migration/upgrade reliability is BUILT (2026-09-21; D-45; docs/v2/evidence/v2-17/README.md): the manual upgrade's before/after inventory (`/api/backup {action:'inventory'}` — every table with counts + the migration ledger), a richer backup summary, and pins that the new V2 state survives a backup→restore cycle exactly (staged restores touch nothing live until applied; the second apply is a no-op; the live credentials file survives; a re-open changes no count); live on the real root: 107 tables, 24-migration ledger, counts matched exactly after a real backup. The carried priority queue (3-10) is now EXHAUSTED; what remains are the roadmap's whole-system phases: V2-15 real dogfood integration (needs Nick's real Fix Capture batches), V2-16 performance/UX polish (largely renderer/Shell), V2-18 synthetic V2 acceptance journeys (NEXT — backend-safe), V2-19 full regression, V2-20 release candidate (needs Nick's promotion gate); V2-05-history stays deferred for Shell integration and the Kibble dev-mission/candidate-model definition stays deferred pending Nick (D-44)
status: partial
