...
next_item: Complete desktop implementation first. Implement and verify the desktop Workspace panel (273:13249) as the next coherent batch. Then close populated Work, Permissions, Projects, and Scheduled tasks with bounded engine states. Pet enable remains locked pending Nick's V1.6 policy decision. Diagnostics maintenance needs matching runtime operations. Keep mobile paused. Update canonical App after desktop completion, then resume mobile UI/UX.
