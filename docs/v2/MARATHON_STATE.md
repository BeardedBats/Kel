...
next_item: Complete desktop implementation first. Finish the isolated package verification for implemented desktop Workspace (273:13249) and File preview (273:13459). Then close populated Work, Permissions, Projects, and Scheduled tasks with bounded engine states. Pet enable remains locked pending Nick's V1.6 policy decision. Diagnostics maintenance needs matching runtime operations. Keep mobile paused. Update canonical App after desktop completion, then resume mobile UI/UX.

status: partial
# Desktop Workspace and File preview are implemented and source-render checked at 1440/800px. Real file search, SCM, read-only mode, split, attachment, save and restoration passed. Final package checks are pending. See evidence/figma-full-audit/DESKTOP_WORKSPACE_CURRENT_REVISION.md.
