...
next_item: Complete desktop implementation first. System, Restore, About, Archived/Select, and enabled WebUI/Change password are package-checked. Next inspect populated Work, Permissions, Projects, and Scheduled tasks with bounded engine states, starting with Work. Desktop Pet enable remains locked pending Nick's V1.6 policy decision; verify its truthful off state separately. Diagnostics maintenance remains open until matching runtime operations exist. Keep mobile paused. Update canonical App for Nick to test after desktop completion, then resume mobile UI/UX.
