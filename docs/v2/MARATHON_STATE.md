...
next_item: acceptance infrastructure (V2-18) # Kibble Build Update is BUILT (2026-09-21; D-46/D-47; docs/v2/evidence/kibble-build-update/README.md): the definition corrected and recorded (Kibble = the user-facing name for Fix Capture / Dogfood; internal identifiers unchanged), the backend contract implemented on the existing machinery (mission = a compile_coding job with verified repo identity + clean baseline + the isolated repositories/<job_id> copy; candidate = a separate record under candidates/<id>/ with revision, verbatim evidence, fixed/unresolved findings, limitations and a human-only review state; promote() always refuses), 10 new tests + a 59-test bounded group green, and a live probe on the real engine (mission created, BUILDING gate held, promote refused, cancel clean, source checkout untouched). NEXT: V2-18 synthetic V2 acceptance journeys (backend-safe); then the remaining whole-system phases (V2-15 needs Nick's real dogfood batches; V2-16 largely renderer/Shell; V2-19 full regression; V2-20 RC whose promotion gate is Nick's). V2-17 upgrade reliability, V2-14 network permissions, V2-13 isolation, V2-12 staffing, V2-11 long-running work, V2-10 learning, V2-09 routing, V2-04b and the V2-04 execution hardening are BUILT (D-34…D-47).
status: partial
