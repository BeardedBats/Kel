...
next_item: Desktop first: implement Update available, shared delete confirmation, then the remaining desktop frames and populated variants. Starting, stopped-engine, and diagnostics export have source evidence; their larger milestone package is pending. Canonical App stays 8c67121 until broader desktop completion. Keep mobile paused. Pet enable needs Nick’s policy decision; diagnostics maintenance needs matching engine operations.
