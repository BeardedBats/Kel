...
next_item: Complete desktop implementation first. Finish rendered and packaged checks for desktop Activity loading (273:12291) and Providers error (273:12486), whose source and focused tests are implemented. Then close populated Work, Permissions, Projects, and Scheduled tasks with bounded engine states. Pet enable remains locked pending Nick's V1.6 policy decision. Diagnostics maintenance needs matching runtime operations. Keep mobile paused. Update canonical App after desktop completion, then resume mobile UI/UX.
