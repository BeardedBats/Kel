...
next_item: migration reliability (Priority 10) # Priority 9 is RESOLVED WITHOUT GUESSING (2026-09-21; D-44; docs/v2/evidence/p9-promotion-gate/README.md): the promotion gate itself was already built (queue_promotion + shadow proposals, recorded-never-applied); this increment added the read-only inspection door on the team surface (action:'promotions' / action:'shadow', project-scoped, honest notes, zero side effects, 5 tests) and DEFERRED the dev-mission schema + candidate model pending Nick's definition of a Kibble Build Update (undefined in the directive; Ramble/Kibble presentation is Astra's Shell lane). Next safe backend item: Priority 10 — Manual upgrade reliability / migration validation (ROADMAP V2-17; directive: safe manual upgrade preserving Projects, conversations, memory, credentials, settings, transcripts, Recipes, Connections and Fix Capture data where appropriate; safe migrations; failure without data loss; practical developer rollback; NO updater infrastructure), reusing the existing backup path (`/api/backup`) and the migration ledger suites; then the queue is empty of backend-safe items (V2-15 dogfood / V2-16 polish / V2-18 acceptance / V2-19 regression / V2-20 RC remain as their own phases)
status: partial
# V2-04 (the Connection Framework) is closed; its two open needs are carried as V2-04a/V2-04b in FEATURE_LEDGER.md.

deferred:
  - "Kibble Build Update dev-mission schema + candidate model: awaiting Nick's definition (D-44; docs/v2/evidence/p9-promotion-gate/README.md). The promotion gate itself is built and now inspectable; the requirement is kept, never removed."
