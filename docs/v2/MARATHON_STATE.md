...
next_item: Complete desktop implementation first. Next implement and compare populated desktop Work 189:907 and task detail 272:739 against current Figma. Keep mobile paused. Finish remaining desktop groups, update the canonical App for Nick to test, then resume mobile UI/UX. Recheck Connections action confirmation when the engine has a mutating catalog action.

status: partial
# Nick prioritised complete desktop implementation on 2026-09-25. He will test desktop while mobile UI/UX implementation continues. Desktop Recipes list, Preview, and Run are implemented with isolated package evidence. Desktop Home and Connections empty/populated/credential states have current scoped package checks. Connections mutating action confirmation is DOM-tested but has no packaged state because the engine catalogs read actions only; the later narrow-label CSS adjustment also awaits package proof. Package by coherent desktop feature groups instead of per Figma frame. Canonical App packages source 8c67121; promotion awaits desktop completion. Current Tools, selected Chat states, Transcriptions, and selected mobile states have scoped disposable package checks. Older visual results are historical. See evidence/figma-full-audit/FIGMA_REVISION_2026-09-25.md and its linked scoped records.
