...
next_item: Desktop first: restore Kibble mission state after navigation/reload and remove its extra panel sheen, then complete retained Tools/chat/task and Light popup/icon states. Setup policy selection/folder persistence/Work-banner integration remain open. The larger Kibble/Setup/Pet/Skills/Light milestone packages 41a73f5; sign-in has archive-browser proof only. Canonical App promotion waits for broader acceptance. Mobile stays paused. Pet enable needs Nick's AUD-MINOR-008 decision; live external acceptance requires credentials.
