...
next_item: Kibble Build Update backend (D-46 definition; docs/v2/evidence/kibble-build-update/README.md) # D-44's deferral is CORRECTED: Kibble is the user-facing name for Fix Capture / Dogfood behavior (internal identifiers unchanged); the authoritative workflow is capture -> select findings -> Build Update -> isolated development mission -> coding runtime repairs Kel's source -> bounded tests/verification -> separate candidate build -> Nick reviews; promotion/installation requires explicit human approval and Build Update never installs. NEXT: implement the backend contract on the reuse map (dogfood_fixes for context; compile_coding + engine + isolated repositories/<job_id> worktree for the mission; code_evidence + manual_review for verification; candidates/<id>/ for the candidate; one additive migration for the domain records; the V2-04a bridge pattern for the runtime entry), with tests proving isolation, denied actions (install/promote), failed tests, unresolved findings on the candidate, and that candidate creation cannot trigger promotion. THEN: V2-18 synthetic acceptance journeys and other backend-safe work. V2-17 upgrade reliability, V2-14 network permissions, V2-13 isolation, V2-12 staffing, V2-11 long-running work, V2-10 learning, V2-09 routing, V2-04b and the V2-04 execution hardening are BUILT (D-34…D-45).
status: partial
