...
next_item: Desktop first: complete retained Tools/chat/task and Light dialog states. Light chat menus now pass source checks; broader icon contrast remains open. Kibble mission recovery/panel sheen now pass source checks; real worker execution remains open. Setup policy selection/folder persistence/Work-banner integration remain open. Latest disposable package is 41a73f5 and predates recovery. Package next by larger milestone; canonical App promotion waits for broader acceptance. Mobile stays paused. Pet enable needs Nick's AUD-MINOR-008 decision; live external acceptance requires credentials.
