...
next_item: V2-19 full V2 regression — the engine suite on this line in bounded groups against one owned stack (start one engine on C:/Users/Nick/KelV2Runs/prepared/engine with the desktop's KEL_PROTECTED_PATHS, run the group, stop it). V2-18 is CLOSED for every backend-reachable §27 row: J-FIX/J-UPGRADE/J-SEC/J-KBU(+negatives)/J-CONV/J-PROJ/J-MEM/J-RECIPE/J-MODEL/J-NET/J-CONN/J-TRANS/J-ACTIVITY/J-WORK/J-ATTN/J-RECOV/J-REMOTE all PASSED on real paths (evidence under docs/v2/evidence/v2-18/runs/, including the FAILED runs that are kept). Two limits are recorded, not passes: (1) no real work reaches VERIFIED on this root because the milestone's manual_review check answers "The reviewer returned no usable assessment." (provider claude) — the engine settles CLOSED/UNCERTAIN with the artifact kept and says so; (2) a request that produces no job (e.g. an unknown recipe) is recorded as DISPATCHED with job_id null and never settles — a shared-contract item in PARALLEL_SHELL_TOUCHES.md. The remaining §27 rows need Shell integration (Astra: phone/renderer) or Nick's real credentials/Muse audio, and are never claimed from a backend journey. # The matrix is docs/v2/evidence/v2-18/ACCEPTANCE_MATRIX.md; the executable journeys are runtime/tools/acceptance_journeys.py (attaches only after proving the recorded pid, its command line and the port owner agree). Kibble Build Update is BUILT (D-46/D-47) and its repair-to-candidate loop is proved end to end (J-KBU C1–C4 with the negatives, D-48/D-49). V2-06/07/08 are BUILT (D-50). V2-15 needs Nick's real dogfood batches; V2-16 is largely renderer/Shell; V2-20 is the release candidate whose promotion gate is Nick's.
status: partial
...
phases:
  ...
  V2-19: partial     # Full V2 regression — the whole runtime/ suite is GREEN on this line: `python -m pytest -q` → 1281 passed, 14 subtests passed in 1143.84 s (19m03s), exit 0, after fixing the one stale expectation D-50 left behind (the recipes step moved 4/v13-recipes → 30/v2-recipe-library) and adding the missing "a store that recorded the old recipes step upgrades additively" case. Desktop slice in this worktree (no install needed): `bunx tsc --noEmit -p tsconfig.json` → exit 0 with 1176 files actually checked, and `bunx vitest run tests/unit` → 36 files / 273 tests passed (27.37s, exit 0). `tests/contract` and `tests/integration` hold no test files here (nothing to run — not a pass). Still outside: the Playwright e2e + phone journeys (Shell stack) and the renderer suites on the integration line (needs a real `bun install`)
