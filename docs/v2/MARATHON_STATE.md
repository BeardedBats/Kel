...
next_item: Desktop first: complete retained Tools/chat/task states, then remaining Light popup/icon states. Kibble worker execution/mission recovery, Setup policy selection/folder persistence/Work-banner integration remain open. Package one larger Kibble/Setup/Pet/sign-in milestone after related desktop implementation, before canonical App promotion. Mobile stays paused. Pet enable needs Nick’s AUD-MINOR-008 decision; live external acceptance requires credentials. Reference inspection is 75/75, not complete desktop acceptance.
