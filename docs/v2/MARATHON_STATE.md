...
next_item: Desktop first: implement Pet off/settings and remote sign-in presentation. Setup source checks passed; its policy selection and Work-banner integration remain open. Kibble build/candidate variants remain open. Runtime dialogs and Ramble now have larger milestone package proof. Finish remaining populated variants and Light-mode labels before canonical App promotion. Keep mobile paused. Pet enable needs Nick’s AUD-MINOR-008 decision. Desktop context inspection is 75/75; accepted parity remains scoped.
