...
Last updated: 2026-09-20 (**D3–D7 landed and verified; D8 next**)
...
- CURRENT HEAD: the D7 commit (see `git log --oneline`; prior: `43cf675` (D6), `a8c27ba` (directive), `dde6c1e` (D5), `e57d4a3` (D4), `78d8f1d` (D3), `b13301a`, `e4d4215`)
- COMPLETED:
  ...
  - **D7 core** — orphaned runs reach Needs Your Attention with the engine's own reason; route-blocked jobs stay auto-resuming; brief reasons preserved + dedupe (this commit)
- COMPLETED PHASES: intake/lane-open · D0 · ENG-001 · D1 · D2 · D3 · D4 · D5 · D6 · D7 (commit list above; tree clean).
- CURRENT PHASE: **D8 — Adaptive staffing** (`MARATHON_DIRECTIVE.md` §D8)
- CURRENT ITEM: D8 — smallest-sufficient-tier staffing with the existing workforce architecture (Commander/Architect/builder/verifier/oracle/red-team), authority ceilings, hidden internally, normal UI says only "Kel is using an independent review".
- EXACT NEXT ACTION: recon the workforce/team architecture (`/api/team`, assignments, derived_state, recipes/pods) and what staffing selection exists today; implement tier selection + ceilings within the existing system; test D0/D1/D2/D3 selection and boundaries; record; commit; then D9.
- REMAINING QUEUE: D8 adaptive staffing → D9 learning promotion → D10 Recipes → D11 cross-device → D12 provider routing → D13 failure recovery → D14 Advanced Activity → D15 security/containment → D16 live capability revision → D17 integration/dev surface → D18 dogfood journeys → D19 full regression → fresh package → installed candidate + final battery. (`ROADMAP.md` keeps the compact table.)
- LATEST TESTS: desktop tsc exit 0 · Vitest 22 files / 213 PASS · engine 1019 OK (unchanged) · D3 real-stack matrix green · D4 live transcription E2E green · D5 transition-core green · D6 live restart/resume verdict all-green · D7 orphan/route-block classification green
