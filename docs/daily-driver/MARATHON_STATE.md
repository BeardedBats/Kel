...
Last updated: 2026-09-20 (**D3–D13 landed and verified; D14 in progress**)
...
- CURRENT HEAD: the D12+D13 commit (see `git log --oneline`; prior: `242f4b8` (D11), `9c35f66` (D10), `6493275` (D9), `82ff501` (D8 closure), `daafd36` (D8 slice 1), `1374fcd` (D7), `43cf675` (D6), `a8c27ba` (directive), `dde6c1e` (D5), `e57d4a3` (D4), `78d8f1d` (D3))
- COMPLETED:
  ...
  - **D12 core** — routing transparency: `/api/state` exposes the engine's `run.claimed` decision per active job; Work page renders one plain sentence (cheapest-eligible, fallback offer, up to 3 skipped providers with translated reasons, honest unknowns); fixed the D11 bodyless-GET gap; live journey all-green (this commit)
  - **D13 core** — remote failure language: gateway codes (`KEL_ENGINE_UNAVAILABLE`/`KEL_ENGINE_UNREACHABLE`) become sentences in the remote browser; network drops read as device problems; raw codes can no longer surface as UI copy (this commit)
- COMPLETED PHASES: intake/lane-open · D0 · ENG-001 · D1 · D2 · D3 · D4 · D5 · D6 · D7 · D8 · D9 · D10 · D11 · D12 · D13 (all committed, tree clean).
- CURRENT PHASE: **D14 — Optional Advanced Activity view** (`MARATHON_DIRECTIVE.md` §D14) — starting recon.
- CURRENT ITEM: D14 recon — inventory what high-level state already exists (`/api/state`: jobs, continuation, approvals, providers, routes; assignments; autonomy) and where an optional Activity surface fits without exposing internals.
- EXACT NEXT ACTION: recon the existing surfaces (Work, Autonomy, Diagnostics) and decide the smallest clean Activity view (likely a page reachable from Work or a section, all user-level, no leases/epochs/IDs).
- REMAINING QUEUE: D14 Activity → D15 security/containment → D16 live capability revision → D17 integration/dev surface → D18 dogfood journeys → D19 full regression → fresh package → installed candidate + final battery. (`ROADMAP.md` keeps the compact table.)
- LATEST TESTS: desktop tsc exit 0 · Vitest 29 files / 245 PASS · engine **1023 OK** (full suite, post-D12) · D3 real-stack matrix green · D4 live transcription E2E green · D5 transition-core green · D6 live restart/resume all-true · D7 orphan/route-block classification green · D8 staffing + team pins green · D9 learning pins + live loop all-true · D10 recipe pins + live loop all-true · D11 gateway unit 4 + pins + real-stack journey all-green · D12 route pins + engine suite + live journey all-green · D13 failure-language pins green
