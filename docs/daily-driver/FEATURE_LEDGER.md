...
| D8 | Adaptive staffing | §18–20 | IN PROGRESS (slice 1) | D8 slice-1 commit | engine ladder verified complete + pinned (`staffing.py` TIERS D0–D4, rules R1–R10, band ceilings, reasons; 8 workforce test files); slice 1 replaced the Work page's internals table with plain language (`staffingLanguage.ts` + `staffing-language.test.ts`); REMAINING: Team-page roster exposure decision + D0–D3 selection/boundary evidence |
