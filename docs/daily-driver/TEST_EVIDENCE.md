...
| 12:0x | D19 residual — donor-org pin (new) | `bunx vitest run tests/unit/donor-org-references.test.ts` | 2 PASS | zero donor-org references in renderer source except the one maintenance comment; the two former links gone |
| 12:1x | D19 residual — full | `bunx tsc --noEmit` · `bunx vitest run` | tsc 0 · 34 files / 260 PASS | after removing the Office-preview install link and the agent-hub contribution link |
| 12:1x | D19 package — built bundle sweep | asar extract + grep over `app.asar` of the final installer | **0 files** contain `iOfficeAI` | renderer bundle included; the previous renderer hit is gone |
| 12:1x | D19 package — installed battery (machine-verifiable) | installed `KelEngine.exe` smoke + metadata + registration | engine live (`1.7.0-dev` answered `/api/state`); installer + uninstaller FileDescription = Kel / 1.7.0-dev; registration points at the candidate; preserved installs verified (V1.6 install restored, engine `f525b15b…`) | GUI-driven probes remain (D0-001, D0-004, D1, D2, upgrade preservation) |
